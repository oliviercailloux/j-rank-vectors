...
public class FitnessNbDominated implements Fitness {
    ...
    /**
     * Invalid iff dirty. When valid, for a given rv x: R(x). As the relation is reflexive, that is never empty. The
     * complement of this set are the rvs incomparable to it or strictly better than it.
     */
    ...
    public FitnessNbDominated(AllRankVectors rvs, Preorder<List<Integer>> preorder, boolean optimistic, boolean weighted) {
	if (weighted) {
	...
	return m_optimistic ? (m_weighted ? FitnessType.OPTIMISTIC_WEIGHTED.toString() : FitnessType.OPTIMISTIC
		.toString()) : (m_weighted ? FitnessType.PESSIMISTIC_WEIGHTED.toString() : FitnessType.PESSIMISTIC
		.toString());
    }
}
