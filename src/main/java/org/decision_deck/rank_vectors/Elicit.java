...
public class Elicit {
	...
	public void export(BinaryRelation<List<Integer>, List<Integer>> relation)
	...
	public void proceed() throws Exception {
		...
		// m_mInterval = ContiguousSet.create(Range.closed(3, 14),
		// DiscreteDomain.integers());
		// m_nInterval = ContiguousSet.create(Range.closed(2, 14),
		// DiscreteDomain.integers());
	...
	public void writeQuestionFile() throws Exception {
		...
		final CharSink sink = Files
				.asCharSink(
						new File("Target=" + m_targetRuleType + ", fit=" + m_fitnessTypes + ", m=" + m_mInterval
								+ ", nbrvs=" + m_nbRvsInterval + " — " + df.format(new Date()) + ".txt"),
						Charsets.UTF_8);
		// final CharSink sink = Files.asCharSink(new File("output.txt"),
		// Charsets.UTF_8);
	...
	public void question(Writer w, AllRankVectors all) throws IOException, InterruptedException {
		...
		// final FitnessNbDominated fitterP = new FitnessNbDominated(all,
		// approxPreorder, false, false);
		// final FitnessNbDominated fitterW = new FitnessNbDominated(all,
		// approxPreorder, false, true);
		...
		ruleComparers
				.add(new RuleComparerWoScore(all, m_targetRule, m_targetWo, approxRule, ScoreType.SUM_SUPPL_WINNERS));
	...
	public void viewParetoGraph() throws Exception {
		...
		// final BinaryRelation<List<Integer>, List<Integer>> pareto =
		// GraphUtils.getTransitiveClosure(paretoNT);
