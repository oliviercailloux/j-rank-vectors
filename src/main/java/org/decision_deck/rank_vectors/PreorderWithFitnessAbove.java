...
/**
 * Considers also the rvs above, suppl to the rvs below the ones whose fitness
 * is computed. Does not seem to be such a brilliant idea.
 * 
 * @author Olivier Cailloux
 * 
 */
public class PreorderWithFitnessAbove extends Preorder<List<Integer>>
		implements BinaryRelation<List<Integer>, List<Integer>> {
	...
	/**
	 * Invalid iff dirty. When valid, for a given rv: all the rvs it is in relation
	 * with (greater than or smaller than or equivalent). As the relation is
	...
	/**
	 * Retrieves the fitness of the given pair: card of Q(x) + card of Q(y) − 2 ×
	 * card(Q(x) inter Q(y)), where Q(x) is the set of rvs connected to (in relation
	 * with) x. Symetric. x = y ⇒ returns 0.
	 * 
	...
	public int getFitness(List<Integer> x, List<Integer> y) {
		updateConnexions();
		final int fit = m_connexions.get(x).size() + m_connexions.get(y).size()
				- 2 * Sets.intersection(m_connexions.get(x), m_connexions.get(y)).size();
		return fit;
	}
