...
public class FitnessLikelihood implements Fitness {
	...
	for (Set<List<Integer>> profile : testProfiles) {
	    ...
	    // assert (togetherWinners.size() == winnersOrdered.size() * (winnersOrdered.size() - 1) / 2);
	}
	final Ordering<Pair<List<Integer>, List<Integer>>> orderByNbOccurences = Ordering.natural().onResultOf(
		new Function<Pair<List<Integer>, List<Integer>>, Integer>() {
	    @Override
