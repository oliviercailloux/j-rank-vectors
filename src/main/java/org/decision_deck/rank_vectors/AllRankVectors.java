...
/**
 * <p>
 * All weakly increasing (thus: non-decreasing) rank-vectors, given a number of
 * alternatives m, a number of voters n. For m=2, n=2, these are the three rank
 * vectors 11, 12, 22.
 * </p>
 * <p>
 * Immutable.
...
public class AllRankVectors {
	...
	/**
	 * Retrieves the rank vectors which are strictly greater, in lexicographic
	 * terms, than the given rank-vector.
	 * 
	 * @param rv must be an element of this set.
	...
	/**
	 * </p>
	 * Retrieves the irreflexive relation of strict pareto-dominance without
	 * transitive edges. Contains only weakly increasing rank-vectors.
	 * </p>
	 * <p>
	 * A rank-vector is in relation with every rank-vector having all the same ranks
	 * except at one position, where it has original rank + 1, except if this would
	 * break weak-increasingness, thus if the original rank at the next position was
	 * the same, and except if the original rank at that position was m.
	 * </p>
	 * <p>
	 * For example, the rank-vector 122 is in relation with 123 and with 222, and
	 * not with 132 as this would break weak-increasingness.
	 * </p>
	 * <p>
	 * The pairs are ordered lexicographically according to the lexicographic order
	 * of each component. Thus a pair p=(p1, p2) appear before p'=(p'1, p'2) iff p1
	 * is lexicographically lower than p'1 or if p1 equals p'1 and p2 is
	...
	/**
	 * Retrieves the number of alternatives, which is the maximal number used in any
	 * position of any rank-vector.
	 * 
	 * @return at least 1.
	 */
	public int getM() {
	...
	/**
	 * Modifies the list in place, replacing the provided sequence of integers by
	 * the next one, considering only weakly increasing (meaning: non decreasing)
	 * sequences. Thus, the sequence 1111 with limit > 2 becomes 1112, the sequence
	 * 223 with limit 3 becomes 233.
	 * 
	 * @param l     not <code>null</code>, may be empty.
	 * @param limit must be ≥ all integers in the provided list.
	 * @return <code>true</code> iff the list has been increased, <code>false</code>
	 *         iff all elements equal the limit.
	 */
	@SuppressWarnings("boxing")
