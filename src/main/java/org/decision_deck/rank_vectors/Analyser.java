...
//	showStats(6, 6);
	...
	allValuesPattern = Pattern.compile("\\b" + m_qNumber + "Q '" + searchString
		+ "': ([0-9\\.; ]+); avg = [0-9\\.]+.");
	...
	for (String line : allLines) {
		...
		if (matcher.find()) {
		    ...
		    final ImmutableList<Double> values = ImmutableList.copyOf(Doubles.stringConverter().convertAll(
			    Arrays.asList(valuesStrings)));
	...
	if (m_intervalSummaries) {
	    ...
	    return skipMin && skipMax ? format.format(average) : format.format(average) + " 〈" + format.format(min)
		    + ", " + format.format(max) + "〉";
